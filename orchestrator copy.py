...
CHANGES FROM v4 MONOLITH:
  ...
  - Convergence detector injects resolution prompt when rabbis loop
...
class DebateOrchestrator:
    ...
    def _check_convergence(self, window=6, threshold=0.40):
        ...
        Looks at the last `window` rabbi exchanges. If the average pairwise
        overlap between Hillel's and Shammai's claims exceeds `threshold`,
        the debate has converged.

        Returns True if converged, False otherwise.
        """
        # Need at least 3 exchanges per side to judge
        recent = [
            ex for ex in self.exchanges[-window * 2:]
            if not ex.get("is_human") and not ex.get("responding_to_human")
        ]
        if len(recent) < 6:
            return False
        ...
        if len(h_texts) < 2 or len(s_texts) < 2:
            return False

        # Check cross-side overlap: are they saying the same things?
        stop = {
            "the", "a", "an", "is", "was", "are", "were", "be", "been",
            "have", "has", "had", "do", "does", "did", "will", "would",
            "shall", "should", "may", "might", "must", "can", "could",
            "this", "that", "these", "those", "not", "no", "but", "and",
            "or", "if", "so", "as", "of", "in", "on", "at", "to", "for",
            "with", "by", "from", "about", "said", "says", "rabbi",
            "hillel", "shammai", "thus", "therefore", "however", "yet",
            "indeed", "segment", "gemara", "sages", "hence", "offering",
        }
        ...
        # Compare each Hillel text against each Shammai text
        overlaps = []
        for h in h_texts[-3:]:
            h_kw = keywords(h)
            if not h_kw:
            ...
            for s in s_texts[-3:]:
                ...
                if not s_kw:
                    continue
                overlap = len(h_kw & s_kw) / min(len(h_kw), len(s_kw))
        ...
        if avg_overlap >= threshold:
            print(f"[CONVERGENCE] Detected: avg cross-side overlap = "
                  f"{avg_overlap:.2f} >= {threshold} over last {len(recent)} "
                  f"exchanges", flush=True)
            return True
    ...
    def step(self):
        ...
        Injects resolution prompt if debate has converged."""
        ...
        # Convergence check: inject resolution if rabbis are looping
        converged = self._check_convergence()
        convergence_note = ""
        if converged:
            from prompts import RESOLUTION_INJECTION
            convergence_note = RESOLUTION_INJECTION.format(opponent=opponent.name)

        reply, seg_ranges = speaker.respond(
            daf_ref=self.daf["ref"],
            segments=self.segments,
            exchanges=self.exchanges[-12:],
            opponent_last=last,
